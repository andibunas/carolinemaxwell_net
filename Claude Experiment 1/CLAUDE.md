...
**Status: PLANNING — no implementation has been approved yet.** This file is the
plan under review. Do not scaffold the app until the user explicitly approves it.
...
## 9. Open Questions for the User (please confirm before build starts)

1. **SEO strategy**: Are you good with plain React Router + a prerendering
   build step + sitemap (per §2), or would you rather revisit using a
   framework with built-in SSR/SSG (e.g. Next.js)? You specified React Router,
   so the plan defaults to the former — just confirming that's intentional
   given the SEO goal.
2. **Layout types**: Does the initial set (`grid`, `featured`,
   `subcategory_list`) cover what you have in mind, or are there specific
   layout behaviors from trangtle.com / lisaknoop.com you want replicated
   (e.g. a full-bleed image slideshow, a masonry layout, hover-reveal
   captions)?
3. **Manifest structure**: one `manifest.json` with both `artworks` and
   `writings` top-level keys, or two separate files? Either works equally
   well with the accessor layer in §5 — just want your preference for
   maintainability.
4. **`/about` landing**: should `/about` itself redirect to Bio, or show its
   own small landing view above the CV/Bio/Contact sub-nav?
5. **Nested breadcrumbs**: for artworks nested more than one level deep,
   should *every* ancestor category be a clickable breadcrumb link, or just
   the top-level section (Artworks/Writings)?
6. **Content authoring**: are you comfortable hand-editing JSON/Markdown files
   for new artworks/writings, or do you eventually want a lightweight admin
   UI / CMS on top of the manifest? (Doesn't change this phase's architecture,
   but worth flagging since the manifest schema above is designed to be
   CMS-friendly either way.)
